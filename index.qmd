...
library(profvis) # for computation speed checks
```
...
fitness_loss_data <-
  tibble(chromosome = c("Autosomal", "X", "Y", "W", "Z", "Cytoplasmic"),
       a_female = c(1, 0.5, NA, 0, 1, 0),
       a_male = c(1, 1, 0, NA, 0.5, 1)) %>% 
...
fitness_loss_m_delta <-
  ...
  mutate(chromosome = fct_relevel(chromosome, c("Autosomal", "X", "Y", "Z", "Cytoplasmic"))) %>% 
  ggplot(aes(x = chromosome, y = 1 - `Inbreeding tolerance`)) +
  geom_col(fill = "#d0e2af", alpha = 0.75) +
    facet_wrap(~Sex, labeller = label_parsed) +
  scale_y_continuous(expand = c(0.0, 0.01), limits = c(0, 1)) + 
...
```
**Figure 2**. female reproductive output at the threshold where the indirect fitness gained by mating with a full-brother ($r = 0.5$) equals the loss in fitness due to inbreeding depression i.e. where chromosome-specific selection should no longer favour inbreeding. Reproductive output is expressed relative to females that exclusively outbreed.    
...
We use these values to inform our parameterisation of inbreeding depression ($\delta$). Individuals that don't inbreed produce offspring with a mean lifespan of 2 time units ($\lambda = 0.5$). For inbred offspring, we model inbreeding depression by increasing the risk of mortality ($\lambda > 0.5$), lowering the mean lifespan of inbred individuals relative to their outbred counterparts.

Each individual possesses an inbreeding locus with two possible alleles. The `A_O` allele encodes inbreeding indifference, whereas the `A_I` allele encodes inbreeding preference. The `number_mutants` argument allows us to set the number of mutations that the population is originally seeded with.

Female-male encounters are found using the mass action law, with male search efficiency $v$. To get an idea of how $v$ influences mating encounters we plot the population wide female-male encounter timestamps. The dashed line shows the mean lifespan of outbred individuals in our simulations. Note that these are just encounters; whether they lead to mating depends upon past mating experiences of both females and males.

```{r}

data <-
  expand_grid(v = c(0.5, 5, 50),
         pop_size = 200) %>% 
  slice(rep(1:n(), each = 200)) %>% 
  mutate(encounter = rexp(600, rate = v))

data %>% 
  ggplot(aes(x = encounter)) +
  geom_histogram(binwidth = 1) +
  coord_cartesian(xlim = c(0, 5)) +
  scale_x_continuous(expand = c(0, 0)) + 
  scale_y_continuous(expand = c(0, 0)) +
  geom_vline(xintercept = 1, linetype = 2) +
  facet_wrap(pop_size ~ v, nrow = 1, strip.position = c("top"),
             labeller = label_glue('N_F = {`pop_size`}, v = {`v`}')) +
  labs(x = "Encounter time") +
  theme_bw() +
...
We consider simple single-locus, two-allele systems for autosomal chromosomes, ZW chromosomes, XY chromosomes and cytoplasmic chromosomes. Inheritance is mendelian, except for cytoplamsic chromosomes, which are exclusively maternally inherited.
...
The function below can be used to test computation speed
...
```{r, eval=FALSE}
# lapply returns output as list

x <- lapply(1:2, continuous_time_simulation, 
       parameters = parameters_autosome, 
       offspring_genotypes_autosome)

# map_dfr returns output as dataframe

if(!file.exists("sim_results/sim_autosomal_results.csv")){
autosomes <- map_dfr(1:16200, continuous_time_simulation,
                    parameters = parameters_autosome,
                    offspring_genotypes_autosome)

write_csv(autosomes, "sim_results/sim_autosomal_results.csv")
}

if(!file.exists("sim_results/sim_X_results.csv")){
X <- map_dfr(1:19200, continuous_time_simulation, parameters = parameters_X, offspring_genotypes_X)

write_csv(X, "sim_results/sim_X_results.csv")
}

if(!file.exists("sim_results/sim_Y_results.csv")){
Y <- map_dfr(1:2700, continuous_time_simulation, parameters = parameters_Y, offspring_genotypes_Y)

write_csv(Y, "sim_results/sim_Y_results.csv")
}

if(!file.exists("sim_results/sim_Z_results.csv")){
Z <- map_dfr(1:19200, continuous_time_simulation, parameters = parameters_Z)

write_csv(Z, "sim_results/sim_Z_results.csv")
}

if(!file.exists("sim_results/sim_W_results.csv")){
W <- map_dfr(1:2700, continuous_time_simulation, parameters = parameters_W, offspring_genotypes_W)

write_csv(W, "sim_results/sim_W_results.csv")
}

if(!file.exists("sim_results/sim_cytoplasmic_results.csv")){
cytoplasmic <- map_dfr(1:5400, continuous_time_simulation, parameters = parameters_C, offspring_genotypes_cytoplasmic)

write_csv(cytoplasmic, "sim_results/sim_cytoplasmic_results.csv")
}
