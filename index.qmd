...
```{r}
make_mating_table(gene_location = "X") %>% 
  select(locus_type, everything()) %>% 
  rename(zygote_type = type) %>% 
  kable() %>% 
  kable_styling() %>% 
...
```{r}
make_mating_table(gene_location = "Y") %>% 
  select(locus_type, everything()) %>% 
  rename(zygote_type = type) %>% 
  kable() %>% 
  kable_styling() %>% 
...
A_allele_tracking <-
  ...
  scale_x_continuous(expand = c(0, 0), limits = c(0, 200), breaks = c(0, 25, 50, 75, 100, 125, 150, 175)) + 
...
A_heatmap <-
  ...
  slice_tail() %>% 
  mutate(D_prop = D*-1,
         Fate = case_when(population_size < 100 ~ "Extinction",
  ...
  #geom_vline(xintercept = 0.66, linetype = 2, colour = "white", linewidth = .8) +
  ...
  facet_wrap(v~sex_expressed, nrow = 2,
             labeller = label_glue('Sex expressing inbreeding allele: {`sex_expressed`}\nMale search efficiency: {`v`}')) +
