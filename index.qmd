...
\begin{align}
  \delta_{i, \mathrm{f}}^* = \frac{rc_{i, \mathrm{m}}F_\mathrm{m} + rc_{i, \mathrm{m}}}{c_{i, \mathrm{f}}F_\mathrm{f} + rc_{i, \mathrm{m}}F_\mathrm{m} + rc_{i, \mathrm{m}} + c_{i, \mathrm{f}}} \label{female_new_threshold}
\end{align}

\begin{align}
  \delta_{i, \mathrm{m}}^* = \frac{c_{i, \mathrm{m}} + c_{i, \mathrm{m}}F_\mathrm{m}}{c_{i, \mathrm{m}} + c_{i, \mathrm{m}}F_\mathrm{m} + rc_{i, \mathrm{f}} + rc_{i, \mathrm{f}}F_\mathrm{f}}. \label{male_new_threshold}    
\end{align}

where $r$ is the genealogical relatedness coefficient, $c_{i, \mathrm{m}}$ and $c_{i, \mathrm{f}}$ are the juvenile class reproductive values of males and females, respectively, and $F_\mathrm{f}$ and $F_\mathrm{m}$ are the female and male inbreeding coefficients. See main text for more details. 
...
Within_organism_conflict_females <-
  ...
  mutate(Case = case_when(c_m == 1 & c_f == 1 & F_m == F_f ~ "Autosome",
         ...
         Inbreeding_freq = case_when(Case == "X" & F_f == 0 |
                                        Case == "Z" & F_m == 0 |
                                        Case == "Autosome" & F_f == 0 ~ "Sporadic",
                                      .default = "Frequent")) %>% 
  filter(!is.na(Case),
         Inbreeding_freq == "Frequent") %>%
  select(-c(c_f, c_m, F_f, F_m, Inbreeding_freq)) %>% 
...
We also ran the simulation across a more specific parameter space, that corresponds to the zone of uncertainty produced by the dynamic effect of inbreeding on relatedness.
...
if(!file.exists("parameters/parameters_fine_X.txt")){
  ...
  parameters_fine_X <- read.table("parameters/parameters_fine_X.txt")
  parameters_fine_Z <- read.table("parameters/parameters_fine_Z.txt")
  parameters_fine_AX <- read.table("parameters/parameters_fine_AX.txt")
  parameters_fine_AZ <- read.table("parameters/parameters_fine_AZ.txt")
}
```
...
**Figure 2**. Results generated by stochastic population genetic simulations. Panels show the conditions where dominant inbreeding alleles invade to a high frequency (0.9) when females (left column) or males (right column) have power over the outcome of mating interactions, for genes found on W, maternally inherited cytoplasmic, X, Autosomal, Z, paternally inherited cytoplasmic and Y chromosomes. Inbreeding depression is expressed as the mean proportion of lifespan lost relative to an outbred individual, while the male refractory period is measured as the proportion of an outbred individual's maximum lifespan that is spent out of the mating pool after mating. The thick dashed lines show $\delta_{i, \mathrm{f}}^*$ and $\delta_{i, \mathrm{m}}^*$ when inbreeding is sporadic, while the thin dashed line shows these thresholds at the frequent inbreeding extreme (if they differ). Note that each pixel reports the outcome of precisely two simulation runs, one where male-female encounters are common ($v = 80$) and another where they are rare ($v = 8$). The mosaic nature of the plot therefore reveals the stochasticity of outcomes when invasion begins with a small number of mutants per run. While inbreeding is therefore never guaranteed to invade, the plot allows identifying parameter regions where it never spreads, as opposed to often doing so. Results are for initial individual population sizes that collectively harbour 4000 gene copies at the relevant locus. Other parameters used in all panels: $C=5$, $n=5$ and $\mu = 1$.  
...
**Figure S3**. simulation results, for levels of inbreeding depression where $\delta_{\mathrm{f}, Z}^*$ and $\delta_{\mathrm{m}, Z}^*$ are predicted to depend on the emergent values of $F_\mathrm{f}$ and $F_\mathrm{m}$. The autosomal situations act as controls, with inbreeding acceptance predicted to invade across a smaller fraction of the plotted space compared with the Z-linked case, if frequent inbreeding affects the invasion dynamics. Note that these panels are zoomed in versions of the relevant panels shown in Figure 2, simulated with a finer resolution. All other details are as in Figure S2.
...
**Figure S5**. Results generated by stochastic population genetic simulations with different dominance coefficients for a X-linked allele. All other details are as in Figure 2. White fill indicates dominance scenarios we do not explore, due to hemizygosity. 
...
**Figure S6**. Results generated by stochastic population genetic simulations with different dominance coefficients for a Z-linked allele. All other details are as in Figure 2. White fill indicates dominance scenarios we do not explore, due to hemizygosity. 
